...
ray.shutdown()
ray.init(num_cpus=1, ignore_reinit_error=True, object_store_memory=800000000)
...
def get_steps_interactive(default_steps):
    ...
    while True:
        try:
            steps_input = input(
                f"\nEnter number of steps (press Enter for default {default_steps}): "
            ).strip()
            if not steps_input:
...
def run_simulation(checkpoint_path=None, seed=None, max_steps=None, interactive=True):
    ...
    print(
        f"Environment: {env_config['n_firms']} firms, {env_config['n_households']} households"
    )
            ...
            household_history.append(
                {
                    ...
                    "employer": (
                        household["employer"] if household["employer"] else "unemployed"
                    ),
                    "wage": household["wage"],
    ...
    households_file = (
        results_dir / f"households_checkpoint{iteration}_seed{seed}_{timestamp}.csv"
    )
    ...
    avg_capital = (
        np.mean([f["capital"] for f in env.firms.values() if not f["bankrupt"]])
        if survivors > 0
        else 0
    )
    ...
    print(
        f"Segments: Budget={segments['budget']}, Mainstream={segments['mainstream']}, Premium={segments['premium']}"
    )
